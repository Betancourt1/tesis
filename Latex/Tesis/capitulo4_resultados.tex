\chapter{Resultados}
    ...
    \section{Análisis del L-espacio: La Red de Infraestructura}
        ...
        \subsubsection{Centralidad de Intermediación}
            ...
            La Figura~\ref{fig:centralidad_b_l_space} identifica los supernodos que concentran simultáneamente volumen de rutas e intermediación para sostener los transbordos del sistema, al escalar por grado ponderado y colorear por centralidad de intermediación; esta lectura habilita priorizar supernodos para protección operativa y mantenimiento preventivo.
            \begin{figure}[h!]
                \centering
        ...
        \subsubsection{Centralidad de Eigenvector (Eigenvector Centrality)}
            ...
            La Figura~\ref{fig:centralidad_c_l_space} muestra en qué corredores coinciden accesibilidad temporal e influencia estructural, al colorear supernodos por cercanía y escalarlos por eigenvector; esta lectura habilita decidir dónde concentrar acciones para reducir tiempos de viaje sin debilitar enlaces críticos.
            \begin{figure}[h!]
                \centering
        ...
        \subsubsection{Resultados de la Detección}
            ...
            La Figura~\ref{fig:comunidades_l_space} muestra qué bloques de supernodos están más cohesionados por rutas compartidas y orienta dónde conviene focalizar seguimiento, con comunidades por modularidad y grosor de arista según rutas compartidas; esta lectura habilita decisiones de monitoreo e intervención por subsistemas.
        ...
        \subsubsection{Resultados del Análisis de Robustez}
            La Figura~\ref{fig:robustez_l_space} compara cómo se deterioran la cobertura (\(LCC\)) y la eficiencia temporal cuando fallan supernodos de alto grado, al contrastar ambas trayectorias bajo el mismo ataque dirigido; esta lectura habilita priorizar decisiones entre resiliencia estructural y tiempos de viaje.
    ...
    \section{Análisis del P-espacio: La Red de Servicio}
        ...
        \subsection{Análisis de Centralidad en el P-espacio}
            ...
            La Figura~\ref{fig:centralidad_b_p_space} identifica los supernodos que concentran los transbordos más críticos y, por tanto, deben mantener prioridad operativa, al escalar por grado y colorear por intermediación en el \(P\)-espacio; esta lectura habilita la priorización de puntos de transferencia.
            ...
            La Figura~\ref{fig:centralidad_c_p_space} muestra desde qué supernodos se alcanza más rápido el resto del servicio y cuáles, además, se insertan en ejes influyentes, al combinar cercanía y eigenvector; esta lectura habilita decidir dónde enfocar mejoras para acortar tiempos de viaje.
        ...
        \subsection{Detección de Comunidades en el P-espacio}
            ...
            La Figura~\ref{fig:comunidades_p_space} muestra qué zonas comparten suficientes rutas para planear acciones coordinadas por comunidad, al representar modularidad y aristas ponderadas por transbordos; esta lectura habilita decisiones de coordinación interzonal.
        ...
        \subsection{Análisis de Robustez del P-espacio}
            ...
            La Figura~\ref{fig:robustez_p_space} compara la pérdida de cobertura y eficiencia cuando fallan los supernodos de mayor grado, al contrastar ambas trayectorias bajo el ataque dirigido; esta lectura habilita priorizar medidas de continuidad operativa.
    ...
    \section{Síntesis comparativa y validación del modelo}
        ...
        \subsection{Sensibilidad de Supuestos de Modelado}
            ...
            La Tabla~\ref{tab:sensibilidad_l_space} compara estructura y \(LCC\) por umbral y muestra qué rangos de consolidación preservan la conectividad del \(L\)-espacio sin inducir fragmentación adicional; esta lectura habilita la decisión del rango de consolidación.
            ...
            La Tabla~\ref{tab:sensibilidad_p_space_route_trip} cuantifica el sesgo que introduce construir el \(P\)-espacio por \texttt{route\_id} frente a \texttt{trip\_id} en cada umbral, mediante el diferencial de aristas exclusivas del modelo por ruta; esta lectura habilita decidir el criterio de construcción más consistente con el objetivo analítico.
        ...
        \subsection{Escenarios de Intervención en la Red Modelada}
            ...
            \begin{itemize}
                \item \textbf{Protección de puntos críticos del sistema:} se asume continuidad reforzada en los 10 puntos agregados que más articulan la red modelada. En términos operativos, esto representa terminales, estaciones de transferencia o corredores cuya interrupción compromete buena parte de la cobertura restante.
            ...
            \end{itemize}
            En los cuatro escenarios se aplicó la misma perturbación severa: eliminación secuencial de los 50 puntos agregados de mayor grado ponderado. Después de la falla se midieron tres indicadores: el porcentaje de puntos remanentes que sigue integrado en la parte principal de la red, la eficiencia global dirigida (ponderada por \texttt{travel\_time\_minutes}) y la longitud del peor recorrido interno, medida como el número máximo de segmentos consecutivos necesarios dentro de la red remanente.

            La Tabla~\ref{tab:intervenciones_topologicas_l_space} compara las métricas post-ataque para cada escenario y muestra qué intervención conserva mejor la cobertura conectada del sistema sin alargar en exceso los recorridos tras la falla.

            \begin{table}[h!]
                \centering
                \small
                \caption{Comparación de intervenciones en la red modelada después de eliminar 50 puntos agregados de mayor grado ponderado. Se reportan las conexiones nuevas del modelo, el porcentaje de puntos remanentes que permanece en la red principal, la accesibilidad temporal conservada y la longitud del peor recorrido interno.}
                \label{tab:intervenciones_topologicas_l_space}
                \resizebox{\textwidth}{!}{%
                \begin{tabular}{lrrrrr}
                    \toprule
                    Escenario & Conexiones nuevas & Red principal (\%) & Accesibilidad conservada (\%) & Peor recorrido & Cambio vs.\ base (pp) \\
                    \midrule
            ...
            \begin{itemize}
                \item \textbf{Protección de puntos críticos} es la mejor opción para mantener integrada la mayor parte de la red remanente: el porcentaje de puntos que sigue dentro de la red principal sube de \(98.51\%\) a \(99.30\%\). Sin embargo, el peor recorrido interno pasa de 84 a 97 segmentos del modelo. En términos de transporte, esto significa que más zonas conservan vínculo con la red principal, pero algunos desplazamientos extremos se vuelven más largos y menos directos cuando parte del sistema falla.
                \item \textbf{Refuerzo periférico} ofrece el mejor balance operativo: mantiene el porcentaje de red principal al nivel del escenario base, pero logra la mayor accesibilidad temporal conservada (\(88.52\%\) frente a \(82.50\%\)) y reduce el peor recorrido interno de 84 a 53 segmentos. En la práctica, esto sugiere que conexiones transversales fuera del centro ayudan a sostener trayectos más razonables y a evitar rodeos excesivos después de una interrupción severa.
                \item \textbf{Conexiones alternativas locales} sólo aporta una mejora marginal en cobertura conectada (\(+0.31\) puntos porcentuales en la red principal), pero reduce la accesibilidad relativa a \(79.68\%\), por debajo del escenario base. En su forma actual, esto indica que abrir rodeos cortos alrededor de puntos cargados no basta si esos rodeos no mejoran de verdad los itinerarios disponibles para los usuarios.
            \end{itemize}
            ...
            La resiliencia del sistema mejora más cuando se refuerzan alternativas útiles de viaje y se asegura la continuidad operativa de los puntos de intercambio más sensibles. Un peor recorrido interno más alto no describe una distancia física mayor, sino que, en el modelo, algunos trayectos pasan a requerir encadenar más segmentos consecutivos; por tanto, representa una red que bajo falla obliga a itinerarios más largos, más indirectos o más frágiles.
        ...
        \subsection{Bandas de Incertidumbre de Resultados}
            ...
            La Tabla~\ref{tab:rangos_consolidados_p3_p4} sintetiza mínimos, máximos y amplitudes y muestra qué resultados permanecen estables y cuáles cambian de forma material según los supuestos del modelo o el tipo de intervención; esta lectura permite identificar qué indicadores requieren monitoreo prioritario.
            ...
            \begin{table}[h!]
                ...
                \begin{tabular}{llrrr}
                    ...
                    \midrule
                    Modelado & Puntos agregados de la red física & 2,676 & 5,084 & 2,408 \\
                    Modelado & Conexiones directas de la red física & 6,280 & 9,550 & 3,270 \\
                    Modelado & Cobertura principal de la red física (\%) & 99.81 & 99.88 & 0.07 \\
                    Modelado & Sobreconectividad del servicio por ruta (\%) & 8.73 & 20.86 & 12.13 \\
                    Intervenciones & Cobertura conectada tras falla (\%) & 98.51 & 99.30 & 0.79 \\
                    Intervenciones & Accesibilidad temporal conservada (\%) & 79.68 & 88.52 & 8.85 \\
            ...
            Los rangos muestran que la \textbf{cobertura principal} de la red física modelada es estable frente a los supuestos de consolidación (\(99.81\%\) a \(99.88\%\)); en cambio, la magnitud de \textbf{sobreconectividad potencial} en la red de servicio asociada a construir por ruta en lugar de por viaje cambia de manera importante (\(8.73\%\) a \(20.86\%\)). Bajo perturbación severa, las intervenciones sí producen diferencias operativas relevantes: la accesibilidad temporal conservada varía entre \(79.68\%\) y \(88.52\%\), y el peor recorrido interno entre 53 y 97 segmentos, lo que modifica de manera material la calidad de los itinerarios remanentes.

            La Figura~\ref{fig:rangos_intervenciones_punto4} compara visualmente los rangos por escenario y muestra que la priorización entre intervenciones puede sostenerse aun bajo incertidumbre estructural, siempre que se interpreten los resultados como bandas y no como valores puntuales.
