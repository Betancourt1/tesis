\chapter{Resultados}
    ...
    \section{Análisis del P-espacio: La Red de Servicio}
        ...
        \subsection{Sensibilidad de Supuestos de Modelado}
            ...
            La pregunta de modelado es: ¿cuánto sesgo introduce construir el \(P\)-espacio por \texttt{route\_id} frente a \texttt{trip\_id} en cada umbral? La Tabla~\ref{tab:sensibilidad_p_space_route_trip} responde con el diferencial de aristas route-only, habilitando decidir el criterio de construcción más consistente con el objetivo analítico.

            \begin{table}[h!]
                \centering
                \caption{Comparación estructural del \(P\)-espacio construido por clique de \texttt{route\_id} frente a clique de \texttt{trip\_id}, con conteo de aristas route-only por umbral. Permite estimar el sesgo de sobreconectividad asociado al supuesto route-only del modelo principal.}
        ...
        \subsection{Escenarios de Intervención Topológica}
            ...
            La pregunta de decisión es: ¿qué intervención topológica ofrece mejor balance entre conectividad, eficiencia y diámetro tras el ataque dirigido? La Tabla~\ref{tab:intervenciones_topologicas_l_space} responde al comparar métricas post-ataque por escenario, habilitando priorizar intervenciones con mayor beneficio operativo.
