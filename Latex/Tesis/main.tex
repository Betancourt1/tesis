...
\chapter{Marco Teórico}
	\section{Fundamentos de la teoría de grafos}
		...
		\subsection{Métricas clave en teoría de grafos}
			...
			\subsection{Grado de un Nodo}
			El grado de un nodo $v$, denotado como $deg(v)$, es el número de aristas incidentes a $v$. Para un grafo no dirigido, se define como:
			\begin{equation}
				deg(v) = |\{ u \in V : \{u,v\} \in E \}|.
			\end{equation}
			En un grafo dirigido, se distinguen el grado de entrada $deg^-(v)$ y el grado de salida $deg^+(v)$:
			\begin{align}
				deg^-(v) &= |\{ u \in V : (u,v) \in E \}|, \\
			...
			\subsection{Densidad del Grafo}
			La densidad de un grafo mide qué tan conectado está en comparación con el máximo posible:
			\begin{equation}
				D = \frac{|E|}{|V|(|V|-1)}.
			\end{equation}
			
			\subsection{Transitividad y Coeficiente de Agrupamiento}
			La transitividad evalúa la presencia de triángulos en un grafo, definida como:
			\begin{equation}
				T = \frac{3 \times \text{número de triángulos}}{\text{número de tríadas abiertas}}.
			\end{equation}
			El coeficiente de agrupamiento de un nodo $v$ mide cuán conectados están sus vecinos:
			\begin{equation}
				C_v = \frac{2T_v}{deg(v)(deg(v)-1)},
			\end{equation}
			siendo $T_v$ el número de triángulos en los que participa $v$.
