...
La presente investigación ha logrado modelar matemáticamente el sistema de transporte de la Zona Metropolitana de Guadalajara (ZMG) mediante una aproximación dual, diferenciando entre la infraestructura física (Espacio $L$) y la red de servicio (Espacio $P$). Este enfoque permitió evaluar una proposición principal única: \textbf{la ZMG combina una base física sensible a fallas con un servicio relativamente robusto, efecto explicado por la redundancia entre rutas.}
...
\begin{enumerate}
    % TODO [PDF p.76][abel][cat: formato][resaltado: "0,1088" en la versión revisada del PDF]: no hay comentario escrito; revisar formato decimal del coeficiente de clustering.
    % TODO [PDF p.76][abel][cat: formato][resaltado: "(~18.50%)" en la versión revisada del PDF]: no hay comentario escrito; revisar consistencia de notación para la caída porcentual.
    ...
    \item \textbf{Verificación de la Tesis Central:} Existe una clara desconexión entre la eficiencia topológica de la infraestructura (baja) y la del servicio (alta). En consecuencia, la evidencia respalda la proposición central del trabajo: \textbf{la ZMG combina una base física sensible a fallas con un servicio relativamente robusto, efecto explicado por la redundancia entre rutas.}
\end{enumerate}
...
% TODO [PDF p.77][abel][cat: conservar][nota flotante sobre: "Tabla de trazabilidad entre preguntas, métricas, resultados, implicaciones y límites"]: conservar esta tabla; el comentario dice "me gusta esta tabla".
\begin{table}[h!]
    \centering
    \scriptsize
...
\begin{enumerate}
    ...
    % TODO [PDF p.79][abel][cat: estructura][nota flotante al pie de página bajo: "Dependencia de datos: recálculo programado de comunidades..."]: revisar comentario general; la parte teórica contiene conceptos que no se usan y puede confundir, aunque quizá sea más costoso quitarlos que mantenerlos.
    % TODO [PDF p.79][abel][cat: reproducibilidad][nota flotante al pie de página bajo: "Dependencia de datos: recálculo programado de comunidades..."]: por transparencia y reproducibilidad, considerar publicar archivos o tablas con los valores correspondientes a los grafos del capítulo 4.
    % TODO [PDF p.79][abel, citando a Fernando Becerra][cat: metodologia][nota flotante al pie de página bajo: "Dependencia de datos: recálculo programado de comunidades..."]: desarrollar más el uso de datos GTFS y formalizar mejor la validación de la tesis central.
\end{enumerate}
