...
El transporte público es la columna vertebral de la movilidad en las grandes urbes modernas. En la Zona Metropolitana de Guadalajara (ZMG), el crecimiento demográfico y la expansión de la mancha urbana han generado una red de transporte extensa y compleja, cuya eficiencia no depende únicamente de la cantidad de unidades o rutas, sino de la estructura subyacente que las conecta. Entender cómo se articulan estas conexiones es fundamental para diagnosticar problemas de movilidad, identificar vulnerabilidades y planificar mejoras estratégicas.

Tradicionalmente, el análisis de transporte se ha enfocado en métricas operativas aisladas (frecuencias, tiempos de recorrido) o en modelos macroscópicos de flujo. Sin embargo, estos enfoques a menudo pasan por alto la arquitectura topológica del sistema: ¿cómo está "tejida" la red? ¿Existen nodos críticos cuya falla colapsaría el servicio? ¿Se agrupan las rutas en comunidades funcionales desconectadas de la geografía administrativa?

La presente tesis aborda estas interrogantes desde la perspectiva de la \textbf{Teoría de Redes Complejas}. A diferencia de enfoques previos que simplifican el sistema a un único modelo, esta investigación adopta una metodología dual basada en \textbf{espacios de representación}: el espacio de infraestructura (Espacio $L$), que modela la red física de paradas y tramos; y el espacio de servicio (Espacio $P$), que modela la red de transferencias y viajes directos desde la perspectiva del usuario.
...
El documento se organiza en seis capítulos que guían al lector desde los conceptos fundamentales hasta los hallazgos empíricos:

\begin{description}
    \item[Capítulo 1: Definiciones.] Establece el marco matemático necesario, introduciendo los conceptos de teoría de grafos, métricas de centralidad y propiedades de redes complejas que se utilizarán a lo largo del trabajo.
    ...
    \item[Capítulo 4: Resultados.] Presenta el análisis cuantitativo de la red de la ZMG. Se exponen las métricas calculadas para los espacios $L$ y $P$, la identificación de nodos centrales, el análisis de comunidades y las pruebas de robustez.
     
    \item[Capítulo 5: Conclusiones.] Sintetiza los hallazgos principales, responde a las preguntas de investigación y propone líneas futuras de trabajo para profundizar en el estudio de la movilidad metropolitana.
\end{description}

Esta investigación aspira a demostrar que las herramientas matemáticas de la ciencia de redes no son solo abstracciones teóricas, sino instrumentos potentes para comprender y mejorar la realidad compleja de nuestros sistemas urbanos.
